...
class TurkAnime : MainAPI() {
    ...
    override suspend fun getMainPage(
        ...
        val home = document.select("div#orta-icerik div.panel")
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return document.select("div#orta-icerik div.panel")
            .mapNotNull { it.toMainPageResult() }
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val title = document.selectFirst("div#detayPaylas div.panel-title")
            ?.text()?.trim() ?: return null

        val poster = fixUrlNull(
            document.selectFirst("div#detayPaylas div.imaj img")?.attr("data-src")
        )

        val description = document.selectFirst("div#detayPaylas p.ozet")
            ?.text()?.trim()

        val year = document.selectFirst("div#detayPaylas a[href*='yil/']")
            ?.attr("href")?.substringAfter("yil/")?.toIntOrNull()

        val tags = document.select("div#animedetay a[href*='anime-turu']")
            .map { it.text() }
        ...
        val episodes = bolumlerDoc.select("div#bolum-list li").mapNotNull {
            val epHref = fixUrlNull(
                it.selectFirst("a[href*='/video/']")?.attr("href")
            ) ?: return@mapNotNull null

            val epName = it.selectFirst("span.bolumAdi")
                ?.text()?.trim() ?: return@mapNotNull null

            val epSeason = 1

            val epTitle = it.selectFirst("a[href*='/video/']")
                ?.attr("title")?.trim() ?: return@mapNotNull null

            val epEpisode = Regex("""(\d+). Bölüm""")
                .find(epTitle)?.groupValues?.get(1)?.toIntOrNull() ?: 1
    ...
    private suspend fun iframe2AesLink(iframe: String): String? {
        ...
        val aesLink = AesHelper.cryptoAESHandler(
            aesData,
            aesKey.toByteArray(),
            false
    ...
    private suspend fun iframe2Load(
        ...
        //     val m3uLink = fixUrlNull(
        //         Regex("""file\":\"([^\"]+)""")
        //             .find(mainAPI)?.groupValues?.get(1)?.replace("\\", "")
        //     )
        ...
        for (button in document.select("button[onclick*='ajax/videosec']")) {
            ...
            val subFrame = fixUrlNull(
                subDoc.selectFirst("iframe")?.attr("src")
            ) ?: continue
